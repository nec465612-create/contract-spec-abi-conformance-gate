...
The exact production alias is measured from before F0 with one Chrome DevTools Network log (`Preserve log` on, `Disable cache` on) plus a read-only page-context observer for EIP-1193 provider `request` calls and `accountsChanged`, `chainChanged`, and `disconnect` events. The observer forwards every original return value/error unchanged and never issues a request. The same capture remains open through F7 and is exported only after the final case; no second tab or second browser session is used.
...
The exported ledger is joined to the explicit action journal by row and chronological sequence. It must contain F0 with zero chain events, every F1–F5 read/retry/provider event, the controlled F6 retained-hash receipt/readback events, and F7 with no hidden polling. A final summary records per-row `plannedMaximum`, `actualChainRequests`, `actualProviderRequests`, `cacheHits`, `cacheMisses`, `retryAttempts`, `retryAfterValues`, `receiptCalls`, `readbackCalls`, `transactionHashes`, `transactionCount`, and `terminalPhase`; every field is concrete or `N/A` by rule, never `UNKNOWN` or `NOT MEASURED`. The HAR, provider-event export, action journal, and summary are retained together and bound to the exact release URL and source HEAD.
