...
Status: PRE_DEPLOY CHANGES REQUIRED. The original frozen Studio run remains historical evidence only; its RPC-STUDIO-001 measurement gap is not being replayed. The corrected runner has one current finalized disposable deployment, but the first case write was blocked by Studio HTTP 429 before submission; the one exact resume attempt was also stopped by the limiter. A post-cooldown sparse finalized read confirms case count `0`. The recovery manifest remains `BLOCKED_PARTIAL`; frontend release evidence remains intentionally pending Vercel E2E.

Revision binding: the historical frozen Studio ledger binds to source commit de66367b459ed421b73bdfb7f3d04bf15088ed38, contract source SHA-256 AA023CABE575E346739C51DA0C49A6C77BE8ED4DB3C035A23AFDFC32D894BE45, chain 61999, contract 0x6de11297EaF221eb95A9E34e5A0e418061789250, and account 0xeF5D2119416A2f5afa35dCFA209766EFC1BE5902. The current recovery state has the same source binding and one finalized disposable deployment at 0xa84f59Fb13056DF9707D4c0aaFFc730ec275afC5 with hash 0xe2307ea2f953e61c8963cce50225b420b5908652622c7396524c5a7773372509. Its manifest is `BLOCKED_PARTIAL`: the first case write and one exact resume attempt hit the 30 requests-per-minute limiter at `eth_sendRawTransaction`, so no case write was accepted; a sparse finalized `get_count` read returned `0`. The superseded deployment with an accepted owner-bound write remains permanently non-resumable. Studio and frontend budgets are separate ledgers. A result in one ledger cannot satisfy the other.
...
The initial measured attempt finalized the deployment before the rate limiter blocked S3. It made exactly 17 counted requests and retained exactly one deployment hash. The one exact resume attempt used a fresh disposable account, made 10 counted requests, retained that same deployment hash, and was blocked at S3 with HTTP 429 and `Retry-After: 46`; no case write was accepted. After cooldown, a separate sparse finalized `get_count` read returned `0`. No further write retry is permitted by the RPC rule; the blocked evidence and sparse read are recorded for the gate.

```text
STUDIO_RUN_CONFIRM=CONTRACT_SPEC_ABI_CONFORMANCE_GATE_STUDIO_MEASURED_RUN STUDIO_RESUME_DEPLOYMENT_HASH=0xe2307ea2f953e61c8963cce50225b420b5908652622c7396524c5a7773372509 STUDIO_RESUME_CONTRACT_ADDRESS=0xa84f59Fb13056DF9707D4c0aaFFc730ec275afC5 node probes/studio_rpc_run.mjs
```

The command above is documented as the only exact recovery command, but the prior attempt is paused after the observed 429; there is no automatic retry. The current runner now validates the exact prior schema/deployment evidence locally and records S1 and S2 as explicit 0-request evidence-reuse rows in resume mode, reducing the pre-S3 resume path from 10 to 7 requests without resubmitting the deployment. A later live attempt requires a fresh exact-revision review and a new observed cooldown; it remains one explicit run, not a retry loop. The `STUDIO_RESTART_PARTIAL_RUN` command is deliberately blocked before RPC for the current `BLOCKED_PARTIAL` manifest; it would create an unnecessary third deployment. The historical rows below remain unchanged and still do not claim measurements from the frozen UI run.

The endpoint is pinned to `https://studio.genlayer.com/api`; any `STUDIO_RPC_ENDPOINT` override that differs from that exact value produces secret-free BLOCKED evidence with zero RPC requests. The instrument also emits one global `rpcRequests` list, operation-local events, row caps, request/operation deadlines, and immediately retained submission hashes. The current deployment, both exact blocked-run ledgers, and superseded accepted-case recovery are recorded in [studio-rpc-recovery-manifest.json](evidence/studio-rpc-recovery-manifest.json). The sparse read is separate read-only corroboration in [studio-rpc-sparse-read-1788606443838.json](evidence/studio-rpc-sparse-read-1788606443838.json); it does not inflate or replace the full-run ledger.
