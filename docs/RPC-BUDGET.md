...
Status: ADAPTED STUDIO E2E PASS and MEASURED VERCEL E2E PASS. The adapted contract computes exact-signature conformance during `freeze_case` and has no LLM, validator prompt, evaluation retry, cooldown, or health monitor. The locked S0–S7 Studio matrix passed with five finalized transactions and one bounded read-only reconciliation; the exact final Vercel release passed the measured F0–F7 journey with two unique verified writes and no replay. Final anonymous review and Explorer submission remain separate gates.
...
| F4 | One create-case write | two wallet-context checks; SDK nonce/gas-estimate/gas-price preparation; one wallet submission; up to 3 finality reads; nonce lookup + version readback | readback keys invalidated before use / 5s cache otherwise | 2 context + 3 preparation + 1 submit + 3 finality + 2 readbacks = 11 observable requests | Finality schedule 5/10/20s; bounded transient retry; one explicit reconciliation after uncertainty | 1 | `SUCCESS`, `FAILED`, `REJECTED`, or `RECONCILIATION_REQUIRED` |
| F5 | One case mutation (`replace_base`, `freeze_case`) | two wallet-context checks; SDK nonce/gas-estimate/gas-price preparation; one wallet submission; up to 3 finality reads; up to 2 next-version readbacks | readback key invalidated before each use / 5s cache otherwise | 2 context + 3 preparation + 1 submit + 3 finality + 2 readbacks = 11 observable requests | Finality schedule 5/10/20s; readbacks at 0/4s; bounded transient retry; no automatic resubmit | 1 | Finalized semantic success, finalized error, or retained reconciliation |
...
The six logical write/finality/readback operations remain one submission, three bounded finality queries, and at most two readbacks. The observable cap is eleven because the current official SDK additionally performs three required transaction-preparation RPCs and the frontend performs two fail-closed wallet-context checks before submission. These calls are measured explicitly rather than hidden or treated as free. The 5/10/20-second finality schedule does not add a fourth request; F5 spends its already-declared second readback only after a four-second propagation delay when the first next-version readback is not yet authoritative. Transient retry attempts are counted, honor Retry-After, and terminate the journey before a duplicate write can occur. The frontend write coordinator also installs a per-submission one-shot transport guard, so GenLayerJS ABI-mismatch fallback cannot issue a second wallet send; the retained journal entry remains the reconciliation boundary.

F6 controlled reconciliation fixture: after a unique F4/F5 write returns a real hash and the journal visibly records `SUBMITTED`, interrupt only the finality-read path by reloading the same browser tab before a terminal result is shown. Do not submit another write. After reload, use the explicit journal `Reconcile` action with the same separate wallet/account context. If the write reaches terminal finality before the reload, retain the hash but do not claim F6 was exercised; perform only the explicit reconciliation path when an unresolved journal entry remains. F6 evidence must show the retained hash, one receipt lookup, at most two authoritative readbacks, terminal `VERIFIED` or `FINALIZED_ERROR`, and zero second wallet submissions.

## FRONTEND RPC BUDGET EVIDENCE

### Vercel E2E capture protocol

The exact production alias is measured from before F0 with the release-owned RPC evidence ledger. `Start E2E capture` clears the prior ledger and reloads the same tab; construction alone issues no request. The ledger wraps the shared GenLayer client request boundary and the selected EIP-1193 provider, forwards every original return value/error unchanged, records no request parameters or account results, and observes `accountsChanged`, `chainChanged`, and `disconnect` without issuing a request. The same capture remains active through F7 and `Export RPC evidence` emits the event list plus concrete F0-F7 summaries. This deterministic in-app capture replaces the unavailable Chrome HAR/CDP surface; the prior blocked-platform artifact and its physical-measurement limitation remain retained rather than rewritten.

Each captured event is normalized to this schema before evidence is accepted: `{seq,row,channel,method,attempt,status,retryAfterMs,cache,txHash,providerEvent,startedAt,durationMs}`. `channel` is `chain` or `provider`; `row` is one of `F0`–`F7`; `attempt` is the actual bounded attempt number; `status` is HTTP/RPC/provider success or error; `retryAfterMs` is the observed header/value or `null`; `txHash` is the returned wallet hash when present; and `providerEvent` is the exact event name or `null`. Cache is `MISS` for a source-defined invalidation/new key, `HIT` only for a same-key read served without a corresponding network event, and `N/A` for provider-only events. Unknown fields are not accepted.
