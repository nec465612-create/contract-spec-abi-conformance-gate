...
Status: PRE_DEPLOY CHANGES REQUIRED. The original frozen Studio run remains historical evidence only; its RPC-STUDIO-001 measurement gap is not being replayed. The corrected runner has one current finalized disposable deployment, but the first case write was blocked by Studio HTTP 429 before submission. The recovery manifest is `BLOCKED_PARTIAL` and awaits refreshed anonymous PRE_DEPLOY review before exact resume; frontend release evidence remains intentionally pending Vercel E2E.

Revision binding: the historical frozen Studio ledger binds to source commit de66367b459ed421b73bdfb7f3d04bf15088ed38, contract source SHA-256 AA023CABE575E346739C51DA0C49A6C77BE8ED4DB3C035A23AFDFC32D894BE45, chain 61999, contract 0x6de11297EaF221eb95A9E34e5A0e418061789250, and account 0xeF5D2119416A2f5afa35dCFA209766EFC1BE5902. The current recovery state has the same source binding and one finalized disposable deployment at 0xa84f59Fb13056DF9707D4c0aaFFc730ec275afC5 with hash 0xe2307ea2f953e61c8963cce50225b420b5908652622c7396524c5a7773372509. Its manifest is `BLOCKED_PARTIAL`: the first case write hit the 30 requests-per-minute limiter at `eth_sendRawTransaction`, so no case write was accepted and the deployment is resumable with the exact manifest pair. The superseded deployment with an accepted owner-bound write remains permanently non-resumable. Studio and frontend budgets are separate ledgers. A result in one ledger cannot satisfy the other.
...
The latest measured attempt finalized the deployment before the rate limiter blocked S3. It made exactly 17 counted requests and retained exactly one deployment hash; the blocked `eth_sendRawTransaction` returned HTTP 429 with `Retry-After: 5`, and no case write was accepted. After the exact refreshed PRE_DEPLOY review and observed cooldown, the only permitted next live command is exact resume of this finalized deployment:
...
The `STUDIO_RESTART_PARTIAL_RUN` command is deliberately blocked before RPC for the current `BLOCKED_PARTIAL` manifest; it would create an unnecessary third deployment. Resume reads the manifest binding and revalidates the finalized deployment without resubmitting it. The historical rows below remain unchanged and still do not claim measurements from the frozen UI run.

The endpoint is pinned to `https://studio.genlayer.com/api`; any `STUDIO_RPC_ENDPOINT` override that differs from that exact value produces secret-free BLOCKED evidence with zero RPC requests. The instrument also emits one global `rpcRequests` list, operation-local events, row caps, request/operation deadlines, and immediately retained submission hashes. The current deployment, exact 17-request blocked run, and superseded accepted-case recovery are recorded in [studio-rpc-recovery-manifest.json](evidence/studio-rpc-recovery-manifest.json). Per the RPC rule, no retry was made after HTTP 429; the hash/state was preserved and the next action is the single exact resume command above.
