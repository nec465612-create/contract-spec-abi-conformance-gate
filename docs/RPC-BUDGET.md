...
The command above is documented as the only exact recovery command, but it is paused after the observed 429 per the no-retry rule. The `STUDIO_RESTART_PARTIAL_RUN` command is deliberately blocked before RPC for the current `BLOCKED_PARTIAL` manifest; it would create an unnecessary third deployment. The historical rows below remain unchanged and still do not claim measurements from the frozen UI run.
