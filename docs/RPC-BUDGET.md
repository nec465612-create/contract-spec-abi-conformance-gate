...
The endpoint is pinned to `https://studio.genlayer.com/api`; any `STUDIO_RPC_ENDPOINT` override that differs from that exact value produces secret-free BLOCKED evidence with zero RPC requests. The instrument also emits one global `rpcRequests` list, operation-local events, row caps, request/operation deadlines, and immediately retained submission hashes. The current deployment, all blocked-run ledgers, accepted partial-write evidence, and superseded accepted-case recovery are recorded in [studio-rpc-recovery-manifest.json](evidence/studio-rpc-recovery-manifest.json). The latest replacement-attempt ledger is `studio-rpc-run-1788608380680.json`; it contains one S0 funding request, HTTP 429, no transaction hash, and no state-changing submission. The separate post-cooldown sparse read is [studio-rpc-sparse-read-1788608456363.json](evidence/studio-rpc-sparse-read-1788608456363.json); it confirms the prior case state and does not inflate or replace the full-run ledger.
...
The reviewed replacement attempt was intentionally not retried after the observed `Retry-After: 31` response. Its exact evidence is `studio-rpc-run-1788608380680.json`: S0-funding `1/1`, request sequence `1`, transaction count `0`, and no deployment or case write. After the cooldown window, the separate two-read sparse evidence `studio-rpc-sparse-read-1788608456363.json` confirmed count `1`, revision `2`, `BASE_DRAFT`, and `last_operation=replace_base` on the existing deployment.
