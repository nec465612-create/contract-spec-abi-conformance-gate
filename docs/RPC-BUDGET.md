...
| F6 | Explicit journal reconciliation | wallet reconnect/context after reload; one receipt lookup; required historical/semantic readback | recovery gateway invalidated before readback / 5s cache otherwise | 3 provider context requests + 1 receipt + up to 2 readbacks | One explicit reconciliation; no duplicate submission; stop after authoritative result | 0 | `VERIFIED`, `FINALIZED_ERROR`, or retained `RECONCILIATION_REQUIRED` |
