...
Status: ADAPTED STUDIO E2E PASS. The deterministic-freeze adaptation has five finalized disposable transactions, a structured finalized stale-revision negative control, and a completed read-only S7 reconciliation. No GitHub/Vercel publication or Vercel E2E has occurred.
...
| Adapted live evidence | `docs/evidence/studio-rpc-run-1788642516201.json` (`PASS`, 53 requests, 5 hashes; SHA-256 `A1F24BCF4B68C27DA942A389CEFC07DDDFB7166B4FB9250769E05627E307BBB7`) |
...
| Adapted S7 reconciliation | 4 read-only requests; receipt finalized, current case revision `3` `DONE`/`CONFORMANT`, count `1`, historical revision `1` preserved |
...
| Release boundary | No further write/retry, GitHub/Vercel publication, or Vercel E2E has occurred; exact POST_DEPLOY_TEST review remains separate |

The first adapted run was authorized by the exact-revision PRE_DEPLOY approval. The retained continuation finalized S4 replace, S5 freeze and S6 stale-negative exactly once. S6 is a finalized, consensus-agreed execution ERROR with leader `rollback` payload `STALE_REVISION`; the corrected runner classifies that structured error explicitly. The S7-only pass then performed exactly four read-only requests and confirmed the finalized freeze receipt, current case state, count, and revision-1 history. No write replay occurred. Studio and frontend RPC ledgers remain separate.
