...
Storage: common fields only; domain={}. Base `{requirements:N(8,{id:ID,text:T(384),polarity:'REQUIRED'|'FORBIDDEN'}),abi:N(16,Entry)}` max8 function entries; response={}. Base cap8192; input aggregate8192. No other fields. ABI non-function entries are validated then ignored for conformance, not silently accepted malformed.
...
Type algorithm: parse base token plus zero..4 suffixes `[]` or `[k]` (1≤k≤64, no leading zero). Base whitelist address,bool,string,bytes,bytes1..32,uint/int with width8..256 multiple8; aliases uint/int canonicalize uint256/int256. Base tuple requires components, recursively canonicalize component types and emit `(`+comma join+`)` followed by suffixes in original order. Other bases forbid components. Depth≤4 and total parameter nodes≤32 per entry. Full comparison signature=`name+'('+input types+')'+'->('+output types+'):'+stateMutability`. Separately compute callable_key=name+'('+comma_join(canonical_input_types)+')'. Maintain a set of callable_keys and reject DUPLICATE_CALLABLE before consensus on a repeated key. Return types and mutability never distinguish overload identity, but remain in each full comparison signature and semantic evaluation. Function order frozen, not alphabetically sorted. Example tuple[][2] with components uint,address → `(uint256,address)[][2]`.

Result `{v:1,labels:N(64,'IMPLEMENTS'|'VIOLATES'|'NONE'|'UNKNOWN')}`; length exactly requirements×functions, row-major. Required row: only IMPLEMENTS/NONE/UNKNOWN valid; forbidden row: VIOLATES/NONE/UNKNOWN. All cells compared. Any UNKNOWN→UNRESOLVED; else any forbidden VIOLATES→FORBIDDEN_SURFACE; else any required row with no IMPLEMENTS→MISSING_REQUIRED_SURFACE; else CONFORMANT. No semantic mismatch enum with undefined meaning. Reducer code model: `rows=[labels[i*n:(i+1)*n] for i in range(m)]`; apply precedence above. Outcome controls eligibility only CONFORMANT.
...
| freeze_case(id:u256,expected_revision:u256)->None | primary/LOCK_BASE single-phase | FROZEN, immutable ABI+requirements |
| evaluate_case(id:u256,expected_revision:u256)->None | any/EVALUATE | DONE + CONFORMANT/FORBIDDEN_SURFACE/MISSING_REQUIRED_SURFACE or UNRESOLVED |
| retry_case(id:u256,expected_revision:u256)->None | any/RETRY | same inputs, next accepted attempt |

Constructor/common views exactly as shared. UI `/new`: requirement rows, polarity selector, normalized ABI JSON textarea; live byte/node limits. `/case/:id`: Edit draft, Freeze, matrix view, Evaluate/Retry buttons per state. Outcome copy `Interface matches submitted requirements` / `Forbidden exposure found` / `Required operation missing` / `Semantic correspondence unresolved`. Show “No deployed-code correctness claim.” Studio fixtures: all ignored entry types, overload pair, nested tuple arrays, malformed components, extra keys, REQUIRED row missing, FORBIDDEN row violated, UNKNOWN. Every write table row has happy and wrong-authority/revision/state proof and view check; use shared full deployment order.
